...
bool isUpperBody(AJoint* joint)
{
  if (joint != 0)
  {
    if (joint->getName().find("Spine") != std::string::npos)
    {
      return true;
    }
  }
...
public:
  ...
  void setTargetPositions()
  {
    ...
    for (size_t i = 1; i < mBiped->getNumJoints(); i++)
    {
      ...
      if (isUpperBody(bvhJoint))
      {
        int idx = (i-1)*3 + 3; // 3 ROT DOF PER JOINT + XYZ
        mTargetPositions[idx+0] = euler[0];
        mTargetPositions[idx+1] = euler[1];
        mTargetPositions[idx+2] = euler[2];
      }
    }
  }
